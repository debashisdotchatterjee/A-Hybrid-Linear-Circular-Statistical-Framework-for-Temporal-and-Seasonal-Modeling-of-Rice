...
# Replace "YOUR_FILE_PATH/MONTHLY AVERAGE OF GAZOLE.xlsx - GAZOLE.csv" with the actual path to your CSV file.
file_path <- "MONTHLY AVERAGE OF GAZOLE.xlsx - GAZOLE.csv" 

data <- read_excel("D:/RICE PRICE FORECASTING OF MALDA/MONTHLY AVERAGE OF GAZOLE.xlsx")
...
if(nrow(data_for_plot) > 0) {
  p_rose <- ggplot(data_for_plot, aes(x = angle)) +
    ...
    labs(title = "Rose Diagram with Kernel Density Overlay_GAZ",
         x = "Month", y = "Frequency") +
    theme_minimal()
  print(p_rose)
} else {
...
if(nrow(data) > 0 && nrow(forecast_df) > 0 && !all(is.na(y_min), is.na(y_max))) {
  plot(data$Date, data$`Monthly Average of Price`, type = "p", col = "red", pch = 19,
       ...
       main = "Time Series Plot of Rice Prices_GAZ",
...
if(length(all_x_polar[!is.na(all_x_polar)]) > 0 && length(all_y_polar[!is.na(all_y_polar)]) > 0) {
  ...
  plot(NULL, NULL, # Start with an empty plot
       ...
       main = "Circular Seasonal Plot of Rice Price Time Series_GAZ",
...
if(length(x_obs) > 0 && length(x_grid) > 0 && length(x_forecast) > 0) {
  plot(x_obs, y_obs, col = "red", pch = 19,
       xlab = "X (Price * cos(angle))", ylab = "Y (Price * sin(angle))",
       main = "Circular Plot of Rice Prices_GAZ (Trend Included)",
       asp = 1, # Ensures circular appearance
