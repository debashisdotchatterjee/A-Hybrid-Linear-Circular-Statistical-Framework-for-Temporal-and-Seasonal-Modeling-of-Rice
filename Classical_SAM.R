...
sam_data <- read_excel("D:/RICE PRICE FORECASTING OF MALDA/MONTHLY AVERAGE OF SAMSI.xlsx")


# --- 2. Inspect and Preprocess the Data ---
print("Original SAMSI Data Head:")
print(head(sam_data))
print("Original SAMSI Data Structure:")
str(sam_data)
print("Summary of SAMSI Data (including NAs):")
summary(sam_data)
...
print("Renamed Columns SAMSI Data Head:")
print(head(sam_data))
...
print(paste("Number of NA dates after parsing:", sum(is.na(sam_data$Date))))
# sam_data <- sam_data %>% filter(!is.na(Date)) # Option: remove rows with NA dates

# Sort data by date
sam_data <- sam_data %>% arrange(Date)

# Handle missing values in 'Avg_Price'
# The head() output showed leading NAs. Let's see how many NAs overall.
print(paste("Number of NA values in Avg_Price before handling:", sum(is.na(sam_data$Avg_Price))))
...
# sam_data_no_na_price <- sam_data %>% filter(!is.na(Avg_Price))
...
# Like before, if you need to analyze specific rice varieties, filter `sam_data`
# For example: sam_data_filtered <- sam_data %>% filter(Rice_Variety == "Specific Variety")
...
if(all(is.na(sam_data$Avg_Price))) {
...
full_date_range <- seq(min(sam_data$Date, na.rm = TRUE), max(sam_data$Date, na.rm = TRUE), by = "month")
full_ts_data <- tibble(Date = full_date_range) %>%
  left_join(sam_data %>% select(Date, Avg_Price), by = "Date")
...
sam_data_no_na_price <- sam_data %>% filter(!is.na(Avg_Price))
if(nrow(sam_data_no_na_price) > 0) {
  price_xts <- xts(sam_data_no_na_price$Avg_Price, order.by = sam_data_no_na_price$Date)
...
autoplot(price_ts_imputed) +
  ggtitle("Monthly Average Price (sam_Data) Over Time (Imputed) ") +
  xlab("Year") +
  ylab("Average Price") +
  theme_minimal()
# ggsave("sam_price_timeseries.png")
...
if(length(price_ts_imputed) >= 2 * frequency) { # Needs at least 2 full cycles
  ggsubseriesplot(price_ts_imputed) +
    ggtitle("Seasonal Subseries Plot of Average Price (sam_Data)") +
    ylab("Average Price") +
    theme_minimal()
  # ggsave("sam_price_subseries.png")
} else {
...
ggAcf(price_ts_imputed, lag.max = min(36, length(price_ts_imputed)-1)) + # Adjust lag.max if series is short
  ggtitle("ACF for Average Price (sam_Data)") +
  theme_minimal()
# ggsave("sam_price_acf.png")
...
ggPacf(price_ts_imputed, lag.max = min(36, length(price_ts_imputed)-1)) + # Adjust lag.max
  ggtitle("PACF for Average Price (sam_Data)") +
  theme_minimal()
# ggsave("sam_price_pacf.png")
...
if(exists("price_ts_imputed") && length(price_ts_imputed) >= 2 * frequency && all(is.finite(price_ts_imputed))) {
  ...
  autoplot(decomp_classical) +
    ggtitle("Classical Decomposition of Average Price (sam_Data)") +
    theme_minimal()
  # ggsave("sam_price_classical_decomposition.png")
} else {
...
library(forecast) # Make sure you have this package installed and loaded

# Identify if there are any missing values (optional, but good for verification)
if (any(is.na(price_ts))) {
  print("Missing values detected in price_ts. Imputing...")
  price_ts_imputed <- na.interp(price_ts)
} else {
  print("No missing values detected in price_ts.")
  price_ts_imputed <- price_ts # If no NAs, use the original ts
}

# Now perform STL decomposition on the imputed data
print("Generating STL Decomposition Plot...")
decomp_stl <- stl(price_ts_imputed, s.window = "periodic")
...
gglagplot(price_ts_imputed, lags = min(12, length(price_ts_imputed)-1)) + # Show lags up to 12 months or less
  ggtitle("Lag Plot of Average Price (sam_Data)") +
  theme_minimal()
# ggsave("sam_price_lagplot.png")
...
if(length(price_ts_imputed) >= frequency) { # Needs at least 1 full cycle
  ggseasonplot(price_ts_imputed, year.labels = TRUE, year.labels.left = TRUE) +
    ylab("Average Price") +
    ggtitle("Seasonal Plot: Average Price (sam_Data) by Year") +
    theme_minimal()
  # ggsave("sam_price_seasonplot_year.png")
  
  # Polar seasonal plot
  # ggseasonplot(price_ts_imputed, polar = TRUE) +
  #   ylab("Average Price") +
  #   ggtitle("Polar Seasonal Plot: Average Price (sam_Data)") +
  #   theme_minimal()
} else {
...
# --- 6. Residual Plot (Rplot 9_Sam.jpg) and Residual Data ---
print("Generating Residual Plot (Rplot 9_Sam.jpg)...")
# Plot of residuals from the fitted ARIMA model
autoplot(residuals(fit_arima)) +
  ggtitle("Residual Plot: ARIMA Model for Samsi Market") +
  ylab("Residuals") +
  xlab("Time") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "blue") + # Add a line at zero for reference
  theme_minimal()
ggsave("Rplot 9_Sam.jpg", width = 8, height = 5) # Save the plot as JPG
...
print("--- Summary Statistics for Imputed Average Price (sam_Data) ---")
...
# unique_varieties_sam <- unique(sam_data$Rice_Variety[!is.na(sam_data$Rice_Variety)])
# if (length(unique_varieties_sam) > 0) {
#   first_variety_sam <- unique_varieties_sam[1]
#   print(paste("--- Analyzing specific variety (example):", first_variety_sam, "---"))
#
#   sam_data_variety_subset <- sam_data %>%
#     filter(Rice_Variety == first_variety_sam, !is.na(Avg_Price)) %>%
#     arrange(Date)
#
#   if(nrow(sam_data_variety_subset) > 1) {
#     price_ts_variety_sam <- ts(sam_data_variety_subset$Avg_Price,
#                                start = c(year(min(sam_data_variety_subset$Date)), month(min(sam_data_variety_subset$Date))),
#                                frequency = 12)
#     price_ts_variety_sam <- na.interp(price_ts_variety_sam) # Impute NAs if any within this subset
#
#     autoplot(price_ts_variety_sam) + ggtitle(paste("Price for", first_variety_sam, "(sam_Data)"))
#     # ggsave(paste0("sam_price_",gsub(" ", "_", first_variety_sam),".png"))
#     print(summary(price_ts_variety_sam))
#     # ... and other plots/tables as above for this specific variety
#   } else {
#     print(paste("Not enough data points for variety:", first_variety_sam))
#   }
# }
...
# For example:
# p <- autoplot(price_ts_imputed) + ggtitle("My Time Series Plot (sam_Data)")
# ggsave("my_sam_time_series_plot.png", plot = p)